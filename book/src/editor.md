...
|--|--|---------|
| `scrolloff` | Number of lines of padding around the edge of the screen when scrolling. `0` (default) matches the `vim`/`spacemacs` presets — `H`/`L` reach the literal top/bottom row; set to e.g. `5` for the scroll margins the `helix` preset uses | `0` |
