...
//! `:highlight` overrides) repaints the live zmax editor at startup.
//!
...
async fn vimrc_colorscheme_repaints_live_editor() -> anyhow::Result<()> {
    ...
    let home = std::env::temp_dir().join(format!("zmax-vimrc-e2e-{}", std::process::id()));
    ...
    // zmax's config dir and the `.vimrc` lookup into the temp tree. Isolated
