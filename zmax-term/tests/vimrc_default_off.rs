//! Regression test: zmax must NOT source the user's personal `~/.vimrc` unless
//! the `source-vimrc` setting is explicitly enabled. The default is off — zmax
...
async fn vimrc_not_sourced_by_default() -> anyhow::Result<()> {
    let home = std::env::temp_dir().join(format!("zmax-vimrc-off-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::fs::create_dir_all(home.join(".vim/bundle/Colors/colors"))?;
    std::fs::create_dir_all(home.join(".zmax"))?;
