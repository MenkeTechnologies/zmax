...
fn primary_fragment(app: &zmax_term::application::Application) -> String {
...
fn primary_from(app: &zmax_term::application::Application) -> usize {
    let (view, doc) = zmax_view::current_ref!(app.editor);
...
async fn confirm_case(confirm_keys: &str, expect: &str, why: &'static str) -> anyhow::Result<()> {
    ...
    let check: &dyn Fn(&zmax_term::application::Application) =
...
// same start position, with vim's mandatory trailing newline stripped (zmax
...
// whole as the cursor moves — vim's behavior. The bug this guards: zmax used a
...
// consumes them is unit-tested by `backup_plan` in zmax-view.
#[tokio::test(flavor = "multi_thread")]
...
// Default (unset) keeps zmax's behaviour (continue); with fo set but no `r`,
...
// same as i_CTRL-R in zmax (which already pastes literally). Uses the `.`
...
fn vim_no_sneak() -> AppBuilder {
    let mut editor = zmax_view::editor::Config::default();
...
async fn vim_exusage_lists_ex_commands() -> anyhow::Result<()> {
    // vim `:exusage` shows an overview of Ex commands; zmax lists its typable
...
async fn vim_viusage_lists_normal_commands() -> anyhow::Result<()> {
    // vim `:viusage` shows a Normal-mode command overview; zmax lists its static
    // commands (`{name}\t{doc}`) in a scratch buffer.
    let mut app = vim().with_input_text("#[a|]#bc").build()?;
    test_key_sequence(
        &mut app,
...
async fn vim_balt_sets_alternate_file() -> anyhow::Result<()> {
    ...
    test_key_sequence(
        ...
        Some(&|app| {
            ...
            let cur = zmax_view::doc!(app.editor);
...
async fn vim_command_name_aliases_dispatch() -> anyhow::Result<()> {
    ...
    // so they errored "no such command". They must now dispatch to their zmax
