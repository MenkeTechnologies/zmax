...
use zmax_core::{diagnostic::Severity, test, Selection, Transaction};
use zmax_loader::workspace_trust::WorkspaceTrust;
use zmax_term::{application::Application, args::Args, config::Config, keymap::merge_keys};
use zmax_view::{
    current_ref, doc,
...
impl LineFeedHandling {
    ...
    pub fn apply(&self, text: &str) -> String {
        let line_end = match self {
            LineFeedHandling::Native => zmax_core::NATIVE_LINE_ENDING,
...
pub async fn test_key_sequence_with_input_text<T: Into<TestCase>>(
    ...
    let (view, doc) = zmax_view::current!(app.editor);
...
pub fn test_syntax_loader(overrides: Option<String>) -> zmax_core::syntax::Loader {
    let mut lang = zmax_loader::config::default_lang_config();

    if let Some(overrides) = overrides {
        let override_toml = toml::from_str(&overrides).unwrap();
        lang = zmax_loader::merge_toml_values(lang, override_toml, 3);
    }

    zmax_core::syntax::Loader::new(lang.try_into().unwrap()).unwrap()
}
...
/// against the selection-first keymap and drives the engine through those
/// keys (e.g. `x` = extend line, multi-cursor `s`). Pin the harness to that
/// selection-first keymap so those tests keep validating the engine; vim-specific tests
...
pub fn temp_path(name: &str) -> PathBuf {
    ...
    path.push(format!("zmax-test-{}-{}.{}", std::process::id(), id, ext));
    path
}
...
pub fn test_editor_config() -> zmax_view::editor::Config {
    zmax_view::editor::Config {
        lsp: LspConfig {
            enable: false,
            ..Default::default()
        },
        ...
        auto_save: zmax_view::editor::AutoSave {
            on_change: false,
            focus_lost: false,
            after_delay: zmax_view::editor::AutoSaveAfterDelay {
                enable: false,
                ..Default::default()
            },
        },
...
pub struct AppBuilder {
    args: Args,
    config: Config,
    syn_loader: zmax_core::syntax::Loader,
...
impl AppBuilder {
    ...
    pub fn with_config(mut self, mut config: Config) -> Self {
        let keys = replace(&mut config.keys, zmax_term::keymap::default());
    ...
    pub fn with_lang_loader(mut self, syn_loader: zmax_core::syntax::Loader) -> Self {
