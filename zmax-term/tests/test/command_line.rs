...
// `:echon` is not a native zmax command — it falls through to the embedded
...
async fn tag_stack_jump_next_pop() -> anyhow::Result<()> {
    ...
    fn cur_line(app: &zmax_term::application::Application) -> usize {
...
fn tag_cur_line(app: &zmax_term::application::Application) -> usize {
    let view = app.editor.tree.get(app.editor.tree.focus);
    let doc = app.editor.document(view.doc).unwrap();
    let text = doc.text();
    text.char_to_line(doc.selection(view.id).primary().cursor(text.slice(..)))
...
async fn version_command_shows_version_in_scratch() -> anyhow::Result<()> {
    ...
    test_key_sequence(
        ...
        Some(&|app| {
            ...
            assert!(
                text.contains(&format!("zmax {}", env!("CARGO_PKG_VERSION"))),
                "scratch should contain the build-time zmax version line, got: {text:?}"
            );
            assert!(
...
async fn set_number_toggles_line_numbers_gutter() -> anyhow::Result<()> {
    ...
    fn has_line_numbers(app: &zmax_term::application::Application) -> bool {
        app.editor
            .config()
            .gutters
            .layout
            .iter()
            .any(|g| matches!(g, zmax_view::editor::GutterType::LineNumbers))
    }
    test_key_sequences(
        &mut AppBuilder::new().build()?,
        vec![
...
async fn set_mouse_string_value_applies() -> anyhow::Result<()> {
    // vim `:set mouse=...` is a string option upstream but maps to a zmax bool.
    ...
    test_key_sequence(
        ...
        Some(&|app| {
            ...
            let has_diag = app
                ...
                .layout
                .iter()
                .any(|g| matches!(g, zmax_view::editor::GutterType::Diagnostics));
...
async fn set_maps_showtabline_fileformat_autoread() -> anyhow::Result<()> {
    ...
    test_key_sequence(
        ...
        Some(&|app| {
            ...
            assert!(
                matches!(c.bufferline, zmax_view::editor::BufferLine::Always),
                "showtabline=2 -> bufferline always"
            );
            assert!(
                matches!(
                    c.default_line_ending,
                    zmax_view::editor::LineEndingConfig::LF
                ),
...
async fn set_tabstop_sets_document_tab_width() -> anyhow::Result<()> {
    ...
    test_key_sequence(
        ...
        Some(&|app| {
            assert!(!app.editor.is_err(), "{:?}", app.editor.get_status());
            assert_eq!(zmax_view::doc!(app.editor).tab_width(), 8);
        }),
        false,
    )
    .await?;
...
async fn set_hlsearch_highlights_search_matches() -> anyhow::Result<()> {
    ...
    test_key_sequence(
        ...
        Some(&|app| {
            ...
            let overlay = zmax_term::ui::EditorView::doc_search_highlights(
                &app.editor,
                doc,
                view,
                &app.editor.theme,
...
async fn set_completes_vim_option_names() -> anyhow::Result<()> {
    ...
    let names = |input: &str| -> Vec<String> {
        zmax_term::ui::completers::setting(&app.editor, input)
            .into_iter()
