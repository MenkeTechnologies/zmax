...
fn doc_base_name(doc: &zmax_view::Document) -> String {
    doc.path()
...
/// zmax's own `:set` command through [`with_cx`]. This is the first editor
...
fn install_viml_host_hooks() {
    ...
    // `:map`/`:nmap`/`:nnoremap`/… → the live zmax keymap. vimlrs fires the
    ...
    // scheme file is fully sourced) rebuilds + applies the zmax theme from the
...
pub fn load_init_scripts(cx: &mut compositor::Context) {
    let dir = zmax_loader::config_dir();

    // Emacs Lisp init. zmax's own config-dir `init.el` is always sourced; the
    ...
    // (off by default) — symmetric with `source-vimrc`. zmax is not Emacs and
    // must not silently run a personal init.el. Personal files are sourced first,
    // then zmax's own `init.el` last so a zmax-specific override wins.
    ...
    if let Some(file) = cx.editor.config().source_elisp_file.clone() {
        if !file.trim().is_empty() {
            el_candidates.push(
                zmax_stdx::path::expand_tilde(std::path::Path::new(file.trim())).into_owned(),
            );
        }
    }
    ...
    #[cfg(unix)]
    {
        // Source Vim configuration so zmax honours `:set`/`:map`/`:colorscheme`.
        // Files are sourced in increasing priority — the user's personal config
        // first, then zmax's own `init.vim`, so a zmax-specific override wins.
        ...
        // `source-vimrc` setting is enabled (off by default): zmax is not Vim
        // and must not silently inherit a personal `.vimrc`. zmax's own
        // `init.vim` in the config dir is always sourced — it is an explicit
        // zmax config, not the user's Vim setup.
...
mod tests {
    ...
    fn editor_layer_installs() {
        ...
        assert_eq!(bound("zmax--editor-lisp-loaded"), "t");
    }
    ...
    /// A vimrc's `:highlight` commands are translated into a live zmax theme:
    ...
    fn viml_highlights_build_theme() {
        use zmax_view::theme::{Color, Modifier};
    ...
    /// zmax theme — the real `.vimrc` path, end to end.
    #[cfg(unix)]
    #[test]
    fn viml_colorscheme_file_builds_theme() {
        ...
        let dir = std::env::temp_dir().join(format!("zmax-colo-{}", std::process::id()));
