...
///
/// `eval_capture_with` seeds the variable on the host after the reset that
/// starts every run, so the input is data rather than syntax — nothing is
/// escaped, and a stage cannot be broken by a quote or a `$` in the text it was
/// handed.
#[cfg(unix)]
pub(super) fn filter(code: &str, input: &str) -> Result<String, String> {
    ...
    phplang::eval_capture_with(src, &[("stdin", input)])
