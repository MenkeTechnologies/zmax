...
use zmax_core::syntax::config::{DebugConfigCompletion, DebugTemplate};
use zmax_dap::{self as dap, requests::TerminateArguments};
use zmax_lsp::block_on;
use zmax_view::editor::Breakpoint;
...
use zmax_view::handlers::dap::{breakpoints_changed, jump_to_stack_frame, select_thread_id};
...
fn dap_callback<T, F>(
    jobs: &mut Jobs,
    call: impl Future<Output = zmax_dap::Result<serde_json::Value>> + 'static + Send,
    callback: F,
) where
...
pub fn dap_start_impl(
    ...
    if !cx
        .editor
        .workspace_trust
        .query(&workspace, zmax_loader::workspace_trust::TrustQuery::Dap)
        .is_trusted()
    {
    ...
    args.insert("cwd", to_value(zmax_stdx::env::current_working_dir())?);
...
// a popup; the pure text is built by `zmax_dap::gud_view` (unit-tested).
...
/// `gdb-display-io-buffer`: the inferior's stdio. zmax' DAP path has no
