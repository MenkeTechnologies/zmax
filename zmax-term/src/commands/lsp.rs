...
/// A wrapper around `lsp::Location` that swaps out the LSP URI for `zmax_core::Uri` and adds
...
pub(crate) fn code_actions_for_range(
    doc: &Document,
    range: zmax_core::Range,
    only: Option<Vec<CodeActionKind>>,
    trigger_kind: CodeActionTriggerKind,
) -> Vec<(
    impl Future<Output = Result<Option<Vec<CodeActionOrCommand>>, zmax_lsp::Error>>,
    LanguageServerId,
)> {
    let mut seen_language_servers = HashSet::new();
        ...
        .filter_map(|language_server| {
            ...
            let code_action_context = lsp::CodeActionContext {
                diagnostics: doc
                    ...
                    .iter()
                    .filter(|&diag| {
                        range.overlaps(&zmax_core::Range::new(diag.range.start, diag.range.end))
                    })
...
fn code_action_on_save_step(
    ...
    let build_request = move |editor: &mut Editor| -> Option<Job> {
        ...
        let full_range = zmax_core::Range::new(0, doc.text().len_chars());
...
/// several edits.
type ResolveFuture =
    std::pin::Pin<Box<dyn Future<Output = Result<lsp::CodeAction, zmax_lsp::Error>> + Send>>;
enum ResolveStep {
...
fn goto_impl(editor: &mut Editor, compositor: &mut Compositor, locations: Vec<Location>) {
    let cwdir = zmax_stdx::env::current_working_dir();
...
pub fn safe_delete(cx: &mut Context) {
    ...
    let (from, to) = if primary.from() == primary.to() {
        let w = zmax_core::textobject::textobject_word(
            doc.text().slice(..),
            primary,
...
pub fn rename_symbol(cx: &mut Context) {
    fn get_prefill_from_word_boundary(editor: &Editor) -> String {
        ...
        if primary_selection.len() > 1 {
            ...
            use zmax_core::textobject::{textobject_word, TextObject};
...
fn compute_inlay_hints_for_view(
    ...
    let range = zmax_lsp::util::range_to_lsp_range(
        doc_text,
        zmax_core::Range::new(first_char_in_range, last_char_in_range),
    ...
    let callback = super::make_job_callback(
        ...
        move |editor, _compositor, response: Option<Vec<lsp::InlayHint>>| {
            ...
            for hint in hints {
                let char_idx = match zmax_lsp::util::lsp_pos_to_pos(
                    doc_text,
                    hint.position,
                    offset_encoding,
                ) {
