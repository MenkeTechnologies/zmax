...
use zmax_event::status::StatusMessage;
use zmax_event::{runtime_local, send_blocking};
...
impl Jobs {
    ...
    pub fn new() -> Self {
        ...
        let status_messages = zmax_event::status::setup();
        Self {
        ...
        if j.wait {
            ...
            tokio::spawn(async move {
                match j.future.await {
                    ...
                    Ok(None) => (),
                    Err(err) => zmax_event::status::report(err).await,
                }
            });
        }
    }
