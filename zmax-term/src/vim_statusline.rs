...
//! statusline elements zmax renders.
//!
//! zmax's statusline is a list of typed elements (`StatusLineElement`) split
...
//! zmax has no element for (`%{expr}` evaluation, `%<` truncation markers,
...
use zmax_view::editor::StatusLineElement as El;
...
pub struct VimStatusLine {
    ...
    /// Elements after `%=`.
    pub right: Vec<El>,
    /// The `%`-codes and literals that had no zmax element, in order. `:set`
    /// reports these instead of silently dropping them.
    pub unsupported: Vec<String>,
}

/// The element a vim `%`-code maps to, or `None` when zmax has no equivalent.
        ...
        match code {
            ...
            _ => match element_for(code) {
                Some(el) => {
                    ...
                    // between them is the literal text, which zmax cannot
