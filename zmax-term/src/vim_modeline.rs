...
/// The options whose value zmax *evaluates* rather than stores: a Vimscript
...
pub fn apply_modeline(editor: &mut zmax_view::Editor, doc_id: zmax_view::DocumentId) {
    ...
    if indent_expand.is_some()
        ...
        || readonly.is_some()
    {
        use zmax_core::indent::{IndentStyle, MAX_INDENT};
        if let Some(doc) = editor.document_mut(doc_id) {
