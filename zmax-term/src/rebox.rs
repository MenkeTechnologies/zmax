...
const TEMPLATES: &[(u16, u16, &[&str])] = &[
    ...
    (145, 543, &["XXXXXXXXXXXXX", "X box123456 X", "XXXXXXXXXXXXX"]),
...
fn style(code: u16, comment: &str) -> Result<Parsed, String> {
    ...
    if code < 100 {
        ...
        let (_, w, lines) = template(generic).filter(|(c, _, _)| *c < 100).ok_or_else(unknown)?;
...
mod tests {
    ...
    fn styles_are_numeric_and_cover_the_loop_and_the_language_tables() {
        ...
        for code in DEFAULT_STYLE_LOOP {
            assert!(s.contains(code), "style loop entry {code} is not registered");
        }
    ...
    fn box_unbox_box_round_trips() {
        for code in [71u16, 72, 73, 74, 75, 76, 81, 82, 83, 84, 85, 86] {
            for comment in [";", "#", "//"] {
                ...
                assert_eq!(first, second, "style {code} with {comment:?} did not round trip");
            }
        }
        assert_eq!(unbox(&boxed("alpha\nbeta", 213, "", 10).unwrap()), "alpha\nbeta");
        assert_eq!(unbox(&boxed("alpha\nbeta", 123, "", 10).unwrap()), "alpha\nbeta");
    }
    ...
    fn detect_style_is_stable_for_every_registered_style() {
        ...
        for code in styles() {
            ...
            let Some(found) = detect_style(&text, ";") else {
                let p = style(code, ";").unwrap();
                assert!(!p.has_decoration(), "style {code} was not detected:\n{text}");
                continue;
            };
    ...
    fn center_moves_the_box_without_resizing_it() {
        ...
        assert_eq!(margin_of(&centered.split('\n').collect::<Vec<_>>()), (40 - w) / 2);
        ...
        assert_eq!(margin_of(&center(&text, 2).split('\n').collect::<Vec<_>>()), 0);
    }
