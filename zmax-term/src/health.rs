...
use zmax_core::config::{default_lang_config, user_lang_config};
use zmax_loader::grammar::load_runtime_file;
...
pub fn general() -> std::io::Result<()> {
    ...
    let config_file = zmax_loader::config_file();
    let lang_file = zmax_loader::lang_config_file();
    let log_file = zmax_loader::log_file();
    let rt_dirs = zmax_loader::runtime_dirs();
    ...
    match config.editor.clipboard_provider.name().as_ref() {
        "none" => {
            ...
            writeln!(
                stdout,
                "    {}",
                "https://github.com/MenkeTechnologies/zmax"
                    .red()
                    .underlined()
            )?;
        }
...
pub fn languages_selection() -> std::io::Result<()> {
    let selection = zmax_loader::grammar::get_grammar_names().unwrap_or_default();
    languages(selection)
}
...
fn languages(selection: Option<HashSet<String>>) -> std::io::Result<()> {
    ...
    let trust = zmax_loader::workspace_trust::WorkspaceTrust::fully_trusted();
    ...
    let check_binary_with_name = |cmd: Option<(&str, &str)>| match cmd {
        Some((name, cmd)) => match zmax_stdx::env::which(cmd) {
...
fn probe_parser(grammar_name: &str) -> std::io::Result<()> {
    ...
    match zmax_loader::grammar::get_language(grammar_name) {
...
fn probe_protocols<'a, I: Iterator<Item = (&'a str, &'a str)> + 'a>(
    ...
    for (name, cmd) in server_cmds {
        let (diag, icon) = match zmax_stdx::env::which(cmd) {
