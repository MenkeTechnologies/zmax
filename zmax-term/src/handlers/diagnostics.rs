...
use zmax_core::Uri;
use zmax_event::{cancelable_future, register_hook, send_blocking};
use zmax_lsp::{lsp, LanguageServerId};
...
use zmax_view::handlers::diagnostics::DiagnosticEvent;
use zmax_view::handlers::lsp::{PullAllDocumentsDiagnosticsEvent, PullDiagnosticsEvent};
use zmax_view::handlers::Handlers;
...
impl zmax_event::AsyncHook for PullDiagnosticsHandler {
...
impl zmax_event::AsyncHook for PullAllDocumentsDiagnosticHandler {
        ...
        loop {
            match cancelable_future(futures.next(), &cancel).await {
                ...
                Some(Some((Err(err), DiagnosticProvider::Lsp { server_id, .. }, _))) => {
                    let parsed_cancellation_data = if let zmax_lsp::Error::Rpc(error) = err {
