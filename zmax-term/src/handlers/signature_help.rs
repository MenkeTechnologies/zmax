...
use zmax_event::{cancelable_future, register_hook, send_blocking, TaskController, TaskHandle};
use zmax_lsp::lsp::{self, SignatureInformation};
use zmax_stdx::rope::RopeSliceExt;
use zmax_view::document::Mode;
use zmax_view::events::{DocumentDidChange, SelectionDidChange};
use zmax_view::handlers::lsp::{SignatureHelpEvent, SignatureHelpInvoked};
...
impl zmax_event::AsyncHook for SignatureHelpHandler {
...
fn active_param_range(
    ...
    match &param.label {
        ...
        lsp::ParameterLabel::LabelOffsets([start, end]) => {
            ...
            // but highlighting in zmax is done using byte offset.
...
pub fn show_signature_help(
    ...
    // annoyance (upstream fix: suppress auto signature-help outside insert mode)
