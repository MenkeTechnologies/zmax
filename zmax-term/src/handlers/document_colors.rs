...
use zmax_core::{syntax::config::LanguageServerFeature, text_annotations::InlineAnnotation};
use zmax_event::{cancelable_future, register_hook};
use zmax_lsp::lsp;
use zmax_view::{
...
impl zmax_event::AsyncHook for DocumentColorsHandler {
            ...
            async move {
                let colors: Vec<_> = future
                    ...
                    .filter_map(|color_info| {
                        let pos = zmax_lsp::util::lsp_pos_to_pos(
                            &text,
    ...
    register_hook!(move |event: &mut DocumentDidChange<'_>| {
        ...
        let apply_color_swatch_changes = |annotations: &mut Vec<InlineAnnotation>| {
            event.changes.update_positions(
                annotations
                    .iter_mut()
                    .map(|annotation| (&mut annotation.char_idx, zmax_core::Assoc::After)),
            );
        };
        ...
        if !event.ghost_transaction {
            ...
            zmax_event::send_blocking(&tx, DocumentColorsEvent(event.doc.id()));
        }
