...
use zmax_core::{syntax::config::LanguageServerFeature, Assoc};
...
impl zmax_event::AsyncHook for DocumentLinksHandler {
        ...
        .filter_map(|language_server| {
            ...
            Some(async move {
                ...
                let links: Vec<_> = links
                    .into_iter()
                    .filter_map(|link| {
                        let start = zmax_lsp::util::lsp_pos_to_pos(
                            &text,
                            link.range.start,
                            offset_encoding,
                        )?;
...
pub(super) fn register_hooks(handlers: &Handlers) {
    ...
    register_hook!(move |event: &mut DocumentDidChange<'_>| {
        ...
        if !event.ghost_transaction {
            ...
            zmax_event::send_blocking(&tx, DocumentLinksEvent(event.doc.id()));
        }
