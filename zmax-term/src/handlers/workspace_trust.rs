...
use zmax_loader::workspace_trust::TrustStatus;
use zmax_view::{events::DocumentDidOpen, handlers::Handlers, DocumentId};
...
pub(super) fn register_hooks(_handlers: &Handlers) {
    ...
    register_hook!(move |event: &mut DocumentDidOpen<'_>| {
        ...
        // Stale: `.zmax/` was edited since the user last ran `trust`. LSPs keep
        ...
        if event.editor.workspace_trust.status(&workspace) == TrustStatus::Stale {
            event.editor.set_status(
                "Workspace `.zmax/` config changed since `:workspace-trust`. \
...
Trusted workspaces may load local Zmax config files (`.zmax/*`) and auto-start language servers. \
