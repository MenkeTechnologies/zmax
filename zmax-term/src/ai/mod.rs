//! AI provider abstraction for zmax' AI-agent integration.
//!
...
//! - `ZMAX_AI_PROVIDER` — `anthropic` (default) or `openai`
//! - `ZMAX_AI_MODEL` — overrides the provider's default model
...
/// A chat role. zmax keeps `System` separate from the message list (Anthropic wants it
...
pub trait Provider: Send + Sync {
    ...
    fn agent_turn(
        ...
        Err(format!(
            "{} does not support agent tool-use yet (set ZMAX_AI_PROVIDER=anthropic)",
            self.name()
        ))
    }
}

/// Runtime model override set by the model picker (takes precedence over `ZMAX_AI_MODEL`).
...
/// with `SPC a g`, or default it on with `ZMAX_AI_AUTOCOMPLETE=1`).
...
pub fn autocomplete_enabled() -> bool {
    match AUTOCOMPLETE.load(std::sync::atomic::Ordering::Relaxed) {
        1 => true,
        2 => false,
        _ => std::env::var("ZMAX_AI_AUTOCOMPLETE")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
    }
}
...
/// The configured provider name (env `ZMAX_AI_PROVIDER`, default anthropic).
pub fn provider_name() -> String {
    provider_and_model().0
}

/// Resolve `ZMAX_AI_PROVIDER` / `ZMAX_AI_MODEL` from the environment (model override wins).
fn provider_and_model() -> (String, Option<String>) {
    let provider = std::env::var("ZMAX_AI_PROVIDER")
        .ok()
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "anthropic".to_string());
    let model = MODEL_OVERRIDE.lock().unwrap().clone().or_else(|| {
        std::env::var("ZMAX_AI_MODEL")
            .ok()
            .filter(|s| !s.is_empty())
    });
    (provider, model)
...
pub fn provider() -> Result<Box<dyn Provider>, String> {
    ...
    match provider.as_str() {
        ...
        other => Err(format!(
            "unknown ZMAX_AI_PROVIDER '{other}' (use 'anthropic' or 'openai')"
        )),
    }
}
