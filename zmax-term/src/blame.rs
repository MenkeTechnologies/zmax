...
//!     by `zmax_view`'s gutter (which can't shell out to git itself, so we
...
pub fn ensure_annotate(path: &Path) {
    if !annotate_enabled() || zmax_view::gutter::has_blame_annotate(path) {
        return;
    }
    let lines = blame_lines(path)
        ...
        .collect();
    zmax_view::gutter::set_blame_annotate(path.to_path_buf(), lines);
}
