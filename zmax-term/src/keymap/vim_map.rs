...
fn mode_from_cmd(cmd: &str) -> Option<(Vec<Mode>, bool, bool, bool)> {
    ...
    let modes = match prefix {
        ...
        "o" => vec![Mode::Normal],
        // Cmdline / terminal / lang maps are their own tables in
        // `commands::typed` (`:cmap` / `:tmap` / `:lmap`), not keymap modes.
        "c" | "t" | "l" => return None,
        _ => return None,
    };
...
mod tests {
    ...
    fn mode_prefixes() {
        ...
        // Cmdline, terminal and language maps are not keymap modes: they have
        // their own tables in `commands::typed` (:cmap / :tmap / :lmap), which
        // the prompt, the terminal panel and insert-mode self-insert read.
        assert!(mode_from_cmd("cnoremap").is_none());
