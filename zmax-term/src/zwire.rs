//! Sync the zmax colorscheme to the zwire terminal host's active scheme.
//!
...
//! ```
//!
//! zmax ships a `zgui-<scheme>` theme (plus a `zgui-<scheme>-light` variant)
...
//! editor setting is on, zmax follows that scheme at startup, on
...
//! zmax grafts those colours onto the `zgui-<scheme>` theme's face structure
...
//! still reproduces exactly in zmax; a custom scheme falls back to cyberpunk's
...
//! immediately even while zmax is otherwise idle — no keypress or focus event
//! required.
//!
//! The sync is bidirectional: when the user commits a theme change inside zmax
...
/// zmax themes use (their `[palette]` section — same design system, see
/// `runtime/themes/zgui-midnight.toml`). Only solid-colour vars map; zwire's
/// glow/dim `rgba()` vars have no zmax face and are intentionally omitted.
...
/// Resolve the zmax theme name that mirrors zwire's active scheme, or `None`
...
/// Map a `~/.zwire/global.toml` body to the zmax theme name. Split from the
...
/// Build the palette overrides (zmax palette-key → colour) from a `global.toml`
...
/// Reverse of the scheme mapping: turn a zmax theme name back into a zwire
...
/// ignored, so a non-app-shell zmax theme never gets pushed to zwire.
...
/// Push a committed zmax theme change back to the zwire host by updating
...
fn write_atomic(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    let tmp = path.with_extension("toml.zmax-tmp");
