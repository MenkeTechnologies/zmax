...
use zmax_core::snippets::{ActiveSnippet, RenderedSnippet, Snippet};
use zmax_core::{self as core, chars, fuzzy::MATCHER, Change, Transaction};
use zmax_lsp::{lsp, util, OffsetEncoding};
use zmax_view::{
...
use zmax_view::{graphics::Rect, Document, Editor};
...
impl Completion {
    ...
    fn resolve_completion_item(
        language_server: &zmax_lsp::Client,
        ...
        let response = zmax_lsp::block_on(future);
        match response {
            Ok(item) => Some(item),
            Err(err) => {
