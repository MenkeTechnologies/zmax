...
use zmax_view::{editor::SmartTabConfig, graphics::Rect, Editor};
...
impl<T: Item> Menu<T> {
    ...
    fn recalculate_size(&mut self, viewport: (u16, u16)) {
        ...
        // width) and `pummaxwidth` (maximum). Unset options keep zmax's own
        ...
        if !fits {
            ...
            for i in 0..win_height {
                ...
                if scroll_line <= i && i < scroll_line + scroll_height {
                    ...
                    cell.set_fg(scroll_style.fg.unwrap_or(zmax_view::theme::Color::Reset));
                } else if !render_borders {
                    // Draw scroll track
                    cell.set_symbol(half_block);
                    cell.set_fg(scroll_style.bg.unwrap_or(zmax_view::theme::Color::Reset));
                }
            }
        }
    }
