...
pub struct Ide {
    ...
    lsp_progress: Option<zmax_view::editor::LspProgress>,
...
impl Ide {
    ...
    fn bg_style(&self, theme: &zmax_view::Theme) -> zmax_view::graphics::Style {
    ...
    pub fn goto_run_error_file(&mut self, forward: bool) -> IdeAction {
        ...
        let Some(idx) = zmax_core::compilation::next_file_index(&refs, cur, forward) else {
            return IdeAction::None;
        };
        self.run_error_idx = idx;
        let (path, line) = errors[idx].clone();
    ...
    fn refresh(&mut self, cx: &mut crate::compositor::Context) {
        ...
        self.status_indent = match doc.indent_style {
            zmax_core::indent::IndentStyle::Tabs => "Tab".to_string(),
            zmax_core::indent::IndentStyle::Spaces(n) => format!("{n} sp"),
        };
    ...
    fn render_structure(&mut self, surface: &mut Surface, theme: &zmax_view::Theme) {
    ...
    fn render_toolbar(&mut self, surface: &mut Surface, theme: &zmax_view::Theme) {
    ...
    fn render_bottom(&mut self, surface: &mut Surface, theme: &zmax_view::Theme) {
    ...
    fn render_ci_body(&mut self, surface: &mut Surface, theme: &zmax_view::Theme, body: Rect) {
    ...
    fn render_todo_body(&mut self, surface: &mut Surface, theme: &zmax_view::Theme, body: Rect) {
        let height = body.height as usize;
        if height == 0 {
            return;
        }
    ...
    fn render_debug_body(&mut self, surface: &mut Surface, theme: &zmax_view::Theme, body: Rect) {
        if body.height == 0 {
            return;
        }
        let base = theme.get("ui.text");
    ...
    fn render_git_body(&mut self, surface: &mut Surface, theme: &zmax_view::Theme, body: Rect) {
        let height = body.height as usize;
        if height == 0 {
            return;
        }
    ...
    fn render_marks_body(&mut self, surface: &mut Surface, theme: &zmax_view::Theme, body: Rect) {
        let height = body.height as usize;
        if height == 0 {
            return;
        }
    ...
    fn render_run_body(&mut self, surface: &mut Surface, theme: &zmax_view::Theme, body: Rect) {
        let Some(run) = self.run.clone() else {
            surface.set_stringn(
                body.x,
                body.y,
    ...
    fn render_stripe(&mut self, surface: &mut Surface, theme: &zmax_view::Theme) {
        use crate::ui::rat::{render, to_rat_color};
        use ratatui::symbols::Marker;
        use ratatui::widgets::canvas::{Canvas, Points, Rectangle};
        use zmax_view::graphics::Color as ZColor;
...
/// real zmax operation; unsupported IDE-only items are omitted.
pub(crate) fn file_menu_entries(
    ...
    if !is_dir {
        ...
        e.push(Entry::item_key(
            ...
            "⇧↵",
            move |_c, cx| {
                if let Err(zmax_view::DocumentOpenError::BinaryFile) =
