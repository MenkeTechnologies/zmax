...
//! as a tour of (nearly) every ratatui widget, all blitted onto the zmax
...
//! `zmax_event::request_redraw` so the gauges/charts animate without input.
//!
...
impl Component for DashboardPanel {
    fn handle_event(&mut self, event: &Event, _cx: &mut Context) -> EventResult {
        let key = match event {
            Event::Key(k) => *k,
            Event::Mouse(ev) => {
                use zmax_view::input::MouseButton;
                match ev.kind {
...
impl DashboardPanel {
    fn render_overview(&self, surface: &mut Surface, theme: &zmax_view::Theme, area: Rect) {
        ...
        // This-process card: Table of zmax' own stats
        {
            let inner = card(surface, theme, topc[2], "zmax process");
    ...
    fn render_processes(&self, surface: &mut Surface, theme: &zmax_view::Theme, area: Rect) {
        use ratatui::layout::Constraint;
        use ratatui::style::Modifier as RMod;
...
fn panel_bg_color(theme: &zmax_view::Theme) -> zmax_view::graphics::Color {
    theme
        .get("ui.background")
        .bg
...
fn card(surface: &mut Surface, theme: &zmax_view::Theme, area: Rect, title: &str) -> Rect {
    use ratatui::style::Modifier as RMod;
    use ratatui::text::Span;
...
fn gauge_color(pct: f64, theme: &zmax_view::Theme) -> ratatui::style::Style {
...
fn render_calendar(surface: &mut Surface, theme: &zmax_view::Theme, area: Rect) {
