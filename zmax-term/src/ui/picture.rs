//! Picture — the zmax port of GNU Emacs `picture-mode`, the 2-D ASCII drawing
...
//! unit-tested [`zmax_core::picture::Canvas`], which owns all the model logic
...
//!
//! zmax aliases (kept from before the `C-c` map existed): M-> / M-< / M-^ /
...
use zmax_core::picture::{next_tab_stop, set_tab_stops, Canvas, Dir};
...
impl Component for Picture {
    fn handle_event(&mut self, event: &Event, _cx: &mut Context) -> EventResult {
        ...
        match key {
            ...
            // Drawing direction (picture-movement-*), zmax M- aliases of the
            // `C-c` chords above.
