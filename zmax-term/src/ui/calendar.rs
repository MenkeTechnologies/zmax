//! Calendar — the zmax port of GNU Emacs `calendar-mode`.
//!
//! A full-screen [`Component`] showing a month grid with a movable "point date".
//! All date arithmetic is the pure, unit-tested [`zmax_core::calendar`]; this
...
//!   q/Esc              — exit
//! zmax aliases kept on keys Emacs leaves free: `I` insert a diary entry,
...
/// (`WEEKDAY_ABBR` in `zmax_core::calendar` is the two-letter grid header).
...
/// `calendar-latitude` / `calendar-longitude` variables; zmax has no such
...
pub struct Calendar {
    ...
    diary: Vec<zmax_core::diary::Entry>,
...
impl Calendar {
    ...
    pub fn diary(&self) -> &[zmax_core::diary::Entry] {
        &self.diary
    }
    ...
    /// months forward (negative = back). zmax shows a single month, whose month
    ...
    pub fn scroll_months(&mut self, n: i64) -> Date {
        self.point = zmax_core::calendar::add_months(self.point, n);
        self.point
    }
    ...
    fn count_days_region(&mut self, cx: &mut Context) {
        ...
        let n = zmax_core::calendar::count_days(mark, self.point);
    ...
    fn diary_line(&self, kind: DiaryKind, text: &str) -> Option<String> {
        ...
        let line = match kind {
            ...
            DiaryKind::Block => {
                let start = self.mark?;
                let (a, b) = if zmax_core::calendar::count_days(start, p) <= 0 {
                    (start, p)
                } else {
                    (p, start)
                };
    ...
    fn write_calendar_file(&mut self, name: &str, body: &str, cx: &mut Context) {
        let dir = zmax_stdx::path::home_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
    ...
    fn mayan_goto_long_count(&mut self, text: &str, cx: &mut Context) {
        ...
        self.point = zmax_core::calendar::from_rd(f);
        cx.editor.set_status(format!(
            "Mayan {}.{}.{}.{}.{} = {} {}, {}",
            parts[0],
            parts[1],
    ...
    fn mayan_goto_haab(&mut self, text: &str, forward: bool, cx: &mut Context) {
        ...
        let f = zmax_core::calendar::mayan_next_haab(
            zmax_core::calendar::rd(self.point),
            target,
            forward,
        );
        self.point = zmax_core::calendar::from_rd(f);
    ...
    fn mayan_goto_tzolkin(&mut self, text: &str, forward: bool, cx: &mut Context) {
        ...
        let f = zmax_core::calendar::mayan_next_tzolkin(
            zmax_core::calendar::rd(self.point),
            target,
            forward,
        );
    ...
    fn mayan_goto_round(&mut self, text: &str, cx: &mut Context) {
        ...
        let f = zmax_core::calendar::mayan_next_round(
            zmax_core::calendar::rd(self.point),
...
impl Component for Calendar {
    fn handle_event(&mut self, event: &Event, cx: &mut Context) -> EventResult {
        ...
        match key {
            ...
            // ---- zmax aliases on keys Emacs leaves free in the calendar ----
            ...
            key!('m') => {
                let p = self.point;
                let dim = zmax_core::calendar::days_in_month(p.year, p.month);
                let dates: Vec<Date> = (1..=dim)
                    .map(|d| Date::new(p.year, p.month, d))
                    .filter(|d| zmax_core::diary::has_entry(&self.diary, *d))
                    .collect();
            ...
            key!('d') => {
                let hits = zmax_core::diary::entries_for(&self.diary, self.point);
                let p = self.point;
                if hits.is_empty() {
    ...
    fn render(&mut self, area: Rect, surface: &mut Surface, ctx: &mut Context) {
        ...
        for d in 1..=dim {
            ...
            let has_diary = zmax_core::diary::has_entry(&self.diary, cell_date);
...
mod tests {
    ...
    fn inserted_diary_lines_parse_back_to_the_right_spec() {
        use zmax_core::diary::{parse_line, DateSpec};
    ...
    fn region_day_count_is_inclusive_and_symmetric() {
        ...
        assert_eq!(zmax_core::calendar::count_days(a, b), 8);
