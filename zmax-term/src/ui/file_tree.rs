...
//! editor — no PTY round-trip, since this runs inside zmax itself.
...
mod tests {
    ...
    fn speed_search_filters_to_matching_paths() {
        ...
        let root = std::env::temp_dir().join(format!("zmax_ft_{}", std::process::id()));
    ...
    fn dotfiles_shown_by_default_and_toggle_via_config() {
        let root = std::env::temp_dir().join(format!("zmax_dot_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn reveal_expands_ancestors_and_selects_file() {
        let root = std::env::temp_dir().join(format!("zmax_rv_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn speed_search_ctrl_nav_moves_selection() {
        let root = std::env::temp_dir().join(format!("zmax_cn_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        std::fs::write(root.join("alpha.rs"), "").unwrap();
    ...
    fn nav_jumps_and_collapse_all() {
        let root = std::env::temp_dir().join(format!("zmax_nav_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join("sub")).unwrap();
