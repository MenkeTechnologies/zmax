//! Render ratatui widgets onto zmax's tui `Surface`.
//!
//! ratatui and zmax-tui share the tui-rs lineage, so the `Color`/`Modifier` vocabularies line up.
...
use zmax_view::graphics::{Color, Modifier, Rect};
...
/// zmax `Color` → ratatui `Color` (so widgets can be themed with editor colors).
...
/// zmax `Style` → ratatui `Style`.
pub fn to_rat_style(s: zmax_view::graphics::Style) -> ratatui::style::Style {
...
/// Render a ratatui widget into `area` of the zmax `Surface`.
