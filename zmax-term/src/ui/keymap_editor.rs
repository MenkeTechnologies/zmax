...
//! `~/.zmax/config.toml`. Each entry is (mode, chord, command); saving rebuilds
...
/// Write one `[keys.<mode>]` binding into `~/.zmax/config.toml`, creating the
...
fn all_bindings() -> Vec<(String, String, String)> {
    ...
    for (mode, trie) in &crate::keymap::default() {
        let m = match *mode {
            zmax_view::document::Mode::Normal => "normal",
            zmax_view::document::Mode::Select => "select",
            zmax_view::document::Mode::Insert => "insert",
        };
