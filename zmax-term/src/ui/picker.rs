...
pub struct Injector<T, D> {
    ...
    _redraw: zmax_event::RequestRedrawOnDrop,
}
...
impl<T: 'static + Send + Sync, D: 'static + Send + Sync> Picker<T, D> {
    pub fn stream(
        ...
        let matcher = Nucleo::new(
            Config::DEFAULT,
            Arc::new(zmax_event::request_redraw),
            None,
            matcher_columns,
        );
    ...
    pub fn with_dynamic_query(
        ...
        zmax_event::send_blocking(&handler, event);
            ...
            PathOrId::Path(path) => {
                ...
                if self.preview_cache.contains_key(path) {
                    ...
                    if matches!(preview, CachedPreview::Document(doc) if doc.syntax().is_none()) {
                        zmax_event::send_blocking(&self.preview_highlight_handler, path.clone());
                    }
                        ...
                        if metadata.is_dir() {
                            ...
                            if is_binary {
                                ...
                                let bytes = std::fs::read(&path)
                                    ...
                                    .unwrap_or_default();
                                let text = zmax_core::Rope::from(hex_preview(&bytes).as_str());
                            ...
                            let mut doc = match Document::open(
                                ...
                                Ok(doc) => doc,
                                Err(zmax_view::DocumentOpenError::BinaryFile) => {
