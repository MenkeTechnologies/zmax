...
//!
//! A full-screen [`Component`] over the pure, unit-tested [`zmax_core::rmail`]
...
pub struct Rmail {
    ...
    mime: Vec<zmax_core::rmail::MimeEntity>,
...
impl Rmail {
    ...
    pub fn toggle_mime(&mut self) -> Option<usize> {
        ...
        self.mime = zmax_core::rmail::parse_mime(msg);
        self.mime_cursor = 0;
        self.scroll = 0;
        self.view = View::Mime;
        Some(self.mime.len())
    ...
    pub fn undigestify(&mut self) -> usize {
        ...
        let parts = zmax_core::rmail::undigestify(msg);
        if parts.is_empty() {
            return 0;
        }
    ...
    pub fn epa_decrypt(&mut self) -> Result<usize, String> {
        ...
        let body = self
            ...
            .ok_or("no message")?;
        let (start, end) =
            zmax_core::rmail::pgp_armor_range(&body).ok_or("no OpenPGP armor in this message")?;
    ...
    pub fn redecode_body(&mut self, coding: &str) -> Result<(), String> {
        ...
        let text = zmax_core::rmail::decode_bytes(&raw, coding)
            .ok_or_else(|| format!("unknown coding system: {coding}"))?;
        let fresh = zmax_core::rmail::parse_mbox(&text);
        let idx = self.mailbox.current;
        let body = fresh
            .get(idx)
            .map(|m| m.body.clone())
    ...
    fn mime_key(&mut self, key: zmax_view::input::KeyEvent) -> EventResult {
        match key {
    ...
    fn summary_key(&mut self, key: zmax_view::input::KeyEvent) -> EventResult {
    ...
    fn edit_key(&mut self, key: zmax_view::input::KeyEvent) {
        match key {
    ...
    fn prompt_key(&mut self, key: zmax_view::input::KeyEvent) -> bool {
    ...
    fn run_prompt(&mut self, action: PromptAction, arg: String) {
        ...
        match action {
            ...
            PromptAction::SummaryBySenders => {
                self.open_filtered_summary(zmax_core::rmail::filter_by_senders(
                    &self.mailbox.msgs,
                    &arg,
                ));
            }
            PromptAction::SummaryByRecipients => {
                self.open_filtered_summary(zmax_core::rmail::filter_by_recipients(
                    &self.mailbox.msgs,
                    &arg,
                ));
            }
            PromptAction::SummaryByTopic => {
                self.open_filtered_summary(zmax_core::rmail::filter_by_topic(
                    &self.mailbox.msgs,
                    &arg,
                ));
            }
            PromptAction::SummaryByRegexp => {
                self.open_filtered_summary(zmax_core::rmail::filter_by_regexp(
                    &self.mailbox.msgs,
                    &arg,
                ));
            }
...
impl Component for Rmail {
    fn handle_event(&mut self, event: &Event, cx: &mut Context) -> EventResult {
        ...
        match key {
            ...
            // Bury the reader (rmail-bury, `b`; `z` is the zmax alias) — no
            ...
            // C-o — rmail-output-as-seen (`O` is the zmax alias).
            ctrl!('o') | key!('O') => {
...
pub fn draft_document(editor: &zmax_view::Editor) -> Option<zmax_view::DocumentId> {
    editor
        .documents()
...
fn one_message_mbox(msg: &zmax_core::rmail::Msg) -> String {
