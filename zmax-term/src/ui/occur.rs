//! Occur — the zmax port of GNU Emacs `occur` / `occur-mode` (`M-s o`).
//!
//! A full-screen [`Component`] over the pure, unit-tested
//! [`zmax_core::occur`] engine. `occur` searches the source buffer for a
...
use zmax_view::{graphics::Rect, DocumentId, Editor, ViewId};
...
fn jump(editor: &mut Editor, doc_id: DocumentId, view_id: ViewId, line: usize, col: usize) {
    ...
    doc.set_selection(view_id, zmax_core::Selection::point(pos));
}
