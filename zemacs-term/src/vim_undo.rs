...
use zemacs_view::Document;

#[derive(serde::Serialize, serde::Deserialize)]
struct UndoFile {
    /// Hash of the document text this history corresponds to.
    content_hash: u64,
    history: zemacs_core::history::HistorySnapshot,
}

/// The directory undo files live in (`undo_dir` config, else `~/.zemacs/undo`).
fn undo_dir(configured: &str) -> Option<PathBuf> {
    if !configured.is_empty() {
        return Some(PathBuf::from(shellexpand_home(configured)));
    }
    let home = std::env::var_os("HOME")?;
    Some(PathBuf::from(home).join(".zemacs").join("undo"))
}
...
fn undo_file_path(dir: &Path, file: &Path) -> PathBuf {
    let key = zemacs_stdx::path::canonicalize(file);
