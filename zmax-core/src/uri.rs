...
impl Uri {
    // This clippy allow mirrors zmax_stdx::Url::from_file_path
    #[allow(clippy::result_unit_err)]
    pub fn to_url(&self) -> Result<zmax_stdx::Url, ()> {
        match self {
            Uri::File(path) => zmax_stdx::Url::from_file_path(path),
        }
    }
...
fn convert_url_to_uri(url: &zmax_stdx::Url) -> Result<Uri, UrlConversionErrorKind> {
    if url.scheme() == "file" {
        url.to_file_path()
            .map(|path| Uri::File(zmax_stdx::path::normalize(path).into()))
...
impl TryFrom<zmax_stdx::Url> for Uri {
    type Error = UrlConversionError;

    fn try_from(url: zmax_stdx::Url) -> Result<Self, Self::Error> {
