...
//
// zmax's live `/` search stores the pattern in the `/` register and matches
...
///
/// The flags that change matching in zmax are `regexp`, `word`/`symbol`,
...
/// fold, zmax's only invisible text): open the fold, or skip the match.
...
// derives from the same table but which zmax does not tabulate.
...
///
/// zmax's regex engine has no Emacs symbol-boundary escape (`\_<`/`\_>`), so
