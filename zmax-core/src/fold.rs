...
// `apply_foldignore` lived here. vim does not post-process a level array for
// 'foldignore': `foldlevelIndent` gives an ignored line level -1, "depends on
// surrounding lines", and the fold builder resolves it. That is both simpler and
// different — this version inherited the previous non-ignored line's level,
// which is not the same answer at a block boundary. See
// `crate::fold_tree::foldlevel_indent`.
...
mod tests {
    ...
    // The 'foldignore' test moved with its subject: an ignored line is now
    // UNDEFINED_LEVEL, covered by `crate::fold_tree`'s
    // `blank_and_foldignore_lines_are_undefined_not_inherited`.
