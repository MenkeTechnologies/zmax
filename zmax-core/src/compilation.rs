//! Compilation error list — the zmax port of GNU Emacs `compile` /
...
mod tests {
    ...
    fn parses_rustc_warning_with_column() {
        let e = parse_line("zmax-core/src/lib.rs:10:5: warning: unused variable: `x`").unwrap();
        assert_eq!(e.file, "zmax-core/src/lib.rs");
        assert_eq!(e.line, 10);
    ...
    fn ignores_noise_lines() {
        ...
        assert!(parse_line("   Compiling zmax-core v0.1.0").is_none());
