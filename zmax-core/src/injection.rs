...
//! `~/.zmax/injections.toml`, then project `.zmax/injections.toml`), on top
...
impl InjectionEngine {
    ...
    /// then the project `injections.toml` under `workspace/.zmax` (later scopes
    ...
    pub fn load(config_dir: &std::path::Path, workspace: &std::path::Path) -> Self {
        ...
        let paths = [
            ...
            workspace.join(".zmax").join("injections.toml"),
        ];
        for path in paths {
