//! iCalendar (RFC 5545) export/import for diary entries — the zmax port of
...
/// Emacs `icalendar-uid-format` yields UIDs like `emacs<n>`; zmax tags its own.
const PRODID: &str = "-//zmax//NONSGML zmax diary//EN";
...
pub fn export_entries(entries: &[Entry], anchor: Date) -> Export {
    ...
    for (i, e) in entries.iter().enumerate() {
        ...
        fold_line(&format!("UID:zmax-{}@diary", i + 1), &mut body);
