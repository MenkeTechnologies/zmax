//! Comint — the filesystem/process-free core of the zmax port of GNU Emacs
...
// hold no process or buffer state; the `zmax-term::ui::comint` component calls
...
/// zmax's single-line input model `process_mark` is normally 0, so both cases
