# `zmax-core::text_engine` — round-2 editor-engine batch

Pure-Rust, editor-type-free algorithms in `zmax-core/src/text_engine.rs`, each
...
`zmax-core::region_ops` batch and from the tree-sitter-driven modules
...
| ⭐ `cycle_identifier_case` | Cycle snake → kebab → camel → Pascal → SCREAMING → … | zmax original — beyond Emacs, VS Code, Vim, Sublime, JetBrains, Zed, Helix (they offer discrete conversions, not a cycle) |
| ⭐ `sum_column` | Sum a numeric column across a line block (returns total + count) | zmax original — spreadsheet-style total with no built-in equivalent in the listed editors |

Build/test: `cargo test -p zmax-core` (green; `text_engine` adds 17 tests, taking
the lib suite from 187 → 204). `cargo clippy -p zmax-core --all-targets` is warning-free.
