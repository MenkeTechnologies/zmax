...
#[derive(Debug)]
pub enum Request {
    RunInTerminal(<requests::RunInTerminal as zmax_dap_types::Request>::Arguments),
    StartDebugging(<requests::StartDebugging as zmax_dap_types::Request>::Arguments),
}

impl Request {
    pub fn parse(command: &str, arguments: Option<serde_json::Value>) -> Result<Self> {
        use zmax_dap_types::Request as _;
