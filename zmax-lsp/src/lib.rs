...
pub use zmax_lsp_types as lsp;

use futures_util::stream::select_all::SelectAll;
use slotmap::SlotMap;
use tokio::sync::mpsc::UnboundedReceiver;
use zmax_core::syntax::config::{
...
pub use zmax_core::diagnostic::LanguageServerId;
...
pub enum Error {
    ...
    Unhandled,
    #[error(transparent)]
    ExecutableNotFound(#[from] zmax_stdx::env::ExecutableNotFoundError),
    #[error(transparent)]
...
pub mod util {
    use super::*;
    use zmax_core::line_ending::{line_end_byte_index, line_end_char_index};
    use zmax_core::snippets::{RenderedSnippet, Snippet, SnippetRenderCtx};
    use zmax_core::{chars, RopeSlice};
    use zmax_core::{diagnostic::NumberOrString, Range, Rope, Selection, Tendril, Transaction};
    ...
    pub fn diagnostic_to_lsp_diagnostic(
        doc: &Rope,
        diag: &zmax_core::diagnostic::Diagnostic,
    ...
    pub fn lsp_pos_to_pos(
        ...
        //
        // FIXME: Zmax does not fully comply with the LSP spec for line terminators.
        // The LSP standard requires that line terminators are ['\n', '\r\n', '\r'].
        // Without the unicode-linebreak feature disabled, the `\r` terminator is not handled by zmax.
        // With the unicode-linebreak feature, zmax recognizes multiple extra line break chars
    ...
    pub fn generate_transaction_from_edits(
        ...
        if edits.len() == 1 {
            ...
            if is_document_replacement {
                ...
                return zmax_core::diff::compare_ropes(doc, &new_text);
            }
        }
        ...
        let mut last_end = 0;
        let mut changes: Vec<(usize, usize, Option<zmax_core::Tendril>)> =
...
impl Registry {
    ...
    /// See zmax_view::editor::Editor::refresh_language_servers
...
fn start_client(
    ...
    let (workspace, workspace_is_cwd) = zmax_loader::find_workspace();
...
pub fn find_lsp_workspace(
    ...
    let mut file = if file.is_absolute() {
        ...
        let current_dir = zmax_stdx::env::current_working_dir();
