...
// general utils:
pub fn dap_pos_to_pos(doc: &zmax_core::Rope, line: usize, column: usize) -> Option<usize> {
...
pub fn jump_to_stack_frame(editor: &mut Editor, frame: &zmax_dap::StackFrame) {
    let path = if let Some(zmax_dap::Source {
        path: Some(ref path),
        ..
    }) = frame.source
    {
...
impl Editor {
    pub async fn handle_debugger_message(
        &mut self,
        id: DebugAdapterId,
        payload: zmax_dap::Payload,
    ) -> bool {
        use zmax_dap::{events, Event};
