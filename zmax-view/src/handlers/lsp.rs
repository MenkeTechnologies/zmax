...
use zmax_lsp::util::generate_transaction_from_edits;
use zmax_lsp::{lsp, LanguageServerId, OffsetEncoding};
...
pub enum ApplyEditErrorKind {
    DocumentChanged,
    FileNotFound,
    InvalidUrl(zmax_core::uri::UrlConversionError),
...
impl From<zmax_core::uri::UrlConversionError> for ApplyEditErrorKind {
    fn from(err: zmax_core::uri::UrlConversionError) -> Self {
...
impl Editor {
    ...
    pub fn handle_lsp_diagnostics(
        ...
        if let Some(doc) = doc {
            ...
            let doc = doc.id();
            zmax_event::dispatch(DiagnosticsDidChange { editor: self, doc });
        }
    }
