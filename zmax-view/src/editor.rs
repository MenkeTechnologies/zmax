...
use zmax_loader::workspace_trust::{ImplicitTrustLevel, TrustQuery, WorkspaceTrust};

/// vim `belloff`: the comma list of events that must NOT beep (`all` silences
/// every one). The `:set` option store lives in zmax-term, so the value is
...
/// (`scrollbind` / `cursorbind`, zmax's follow mode) keep in step. `ver` binds
/// vertical scrolling — the only thing zmax's follow mode ever bound — and
/// `hor` binds horizontal scrolling. (`jump` only chooses how the *offset*
/// between two bound windows is recomputed; zmax's follow mode has no
...
use zmax_vcs::DiffProviderRegistry;

use futures_util::stream::select_all::SelectAll;
use futures_util::StreamExt;
use tokio_stream::wrappers::UnboundedReceiverStream;
use zmax_lsp::{Call, LanguageServerId};
...
pub use zmax_core::diagnostic::Severity;
use zmax_core::{
...
use zmax_dap::{self as dap, registry::DebugAdapterId};
use zmax_lsp::lsp;
use zmax_stdx::path::canonicalize;
...
pub struct Config {
    ...
    /// `true` if zmax should automatically add a line comment token if you're currently in a comment
    /// and press `enter`.
    ...
    /// zmax *global* which-key: when true, every pending key sequence pops
    ...
    pub which_key: bool,
    /// The fzf.vim-style commands (`:Files`/`:Colors`/…), whose picker is the
    /// embedded arb running in-process: popup size/layout options, and the
    ...
    pub vim_sneak: bool,
    /// When true, zmax sources the user's personal Vim configuration
    /// (`~/.vimrc`, `~/.vim/vimrc`, `~/.config/nvim/init.vim`) at startup,
    /// honouring its `:set`/`:map`/`:colorscheme`. **Defaults to false** — zmax
    /// is not Vim and does not read your personal Vim config unless you opt in.
    /// (zmax's own `init.vim` in the config dir is always sourced regardless.)
    pub source_vimrc: bool,
    /// When true, zmax sources the user's personal Emacs configuration
    /// (`~/.emacs.d/init.el`, `~/.config/emacs/init.el`, `~/.emacs`) at startup,
    /// running its Emacs Lisp. **Defaults to false** — zmax is not Emacs and
    /// does not run your personal init.el unless you opt in. (zmax's own
    ...
    /// Defaults to `false` (vim's default, and zmax never beeped).
    pub error_bells: bool,
    /// vim `visualbell`: use a visual bell instead of beeping — zmax shows the
    ...
    /// `true` — zmax has always carried the indent over.
    ...
    /// to false.
    pub title: bool,
    /// vim `titlestring`: the title text; empty uses `<file> - zmax`. `%f`/`%t`
    ...
    /// Follow the zwire terminal host's active colorscheme. When true, zmax
...
pub enum ImplicitTrustLevelConfig {
    ...
    None,
    /// Trust Zmax-launched server processes (LSP and DAP) implicitly. Workspace-local config and
    ...
    /// expectations while the workspace-controlled `.zmax/` config still requires opt-in.
    #[default]
    Servers,
...
impl From<&WorkspaceTrustConfig> for zmax_loader::workspace_trust::Config {
    fn from(v: &WorkspaceTrustConfig) -> Self {
        Self {
            level: v.level.into(),
            prompt: v.prompt,
            trusted_globs: zmax_loader::workspace_trust::build_trusted_globs(&v.trusted),
        }
    }
}
...
/// What to open when zmax is launched with no file arguments.
#[derive(Debug, Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
/// The running zmax server (emacs `server-start`).
#[cfg(unix)]
...
/// `options` as extra CLI flags, then runs `sink` — a zmax `:` command line
...
impl Default for FzfConfig {
    fn default() -> Self {
        // Empty by default so zmax adds NOTHING that would override the user's
        ...
        // in to zmax-provided defaults when you have no env config.
        Self {
            options: Vec::new(),
            preview: String::new(),
...
/// close and cross-file jumps, and round-trip through `.zmaxinfo`. Position is
...
//
// The `:set` option store lives in zmax-term, so both values are pushed down
...
pub struct Editor {
    ...
    /// seeded across sessions from `.zmaxinfo`.
    ...
    /// Set by `:set virtualedit=…`; the option store lives in zmax-term, so the
    ...
    pub picture_dir: zmax_core::picture::Dir,
    ...
    pub language_servers: zmax_lsp::Registry,
    ...
    /// close, jump across files, and round-trip through `.zmaxinfo`. Numbered
    ...
    /// One-shot: this insert session used several cursors so the typed text would
    /// land on every line, but vim ends it with a single cursor. Set by the entry
    /// points that fan out — `open` (`3o`/`2O`) and block insert/append
    /// (`<C-v>I`/`<C-v>A`) — each of which aims the primary at vim's resting
    /// cursor first: the last-opened line for a counted open, the block's top row
    /// for a block insert. `enter_normal_mode` consumes it and collapses to that
    /// primary.
...
impl Editor {
    pub fn new(
        ...
        let language_servers = zmax_lsp::Registry::new(syn_loader.clone());
        ...
        Self {
            ...
            auto_fill: false,
            picture_mode: false,
            picture_dir: zmax_core::picture::Dir::E,
    ...
    pub fn refresh_config(&mut self, old_config: &Config) {
        ...
        self._refresh();
        zmax_event::dispatch(crate::events::ConfigDidChange {
            editor: self,
            old: old_config,
            new: &config,
        })
    ...
    /// visual bell instead of an audible one — zmax already shows the error on
    ...
    pub fn move_path(&mut self, old_path: &Path, new_path: &Path) -> io::Result<()> {
        ...
        for language_server in language_servers {
            ...
            let edit = match zmax_lsp::block_on(request) {
        ...
        let language_servers = lang.as_ref().map_or_else(HashMap::default, |language| {
            self.language_servers
                ...
                .filter_map(|(lang, client)| match client {
                    ...
                    Err(err) => {
                        if let zmax_lsp::Error::ExecutableNotFound(err) = err {
    ...
    pub fn new_file_from_stdin(&mut self, action: Action) -> Result<DocumentId, Error> {
        ...
        doc.ensure_view_init(view.id);
        let transaction =
            zmax_core::Transaction::insert(doc.text(), doc.selection(view.id), stdin.into())
    ...
    pub fn open(&mut self, path: &Path, action: Action) -> Result<DocumentId, DocumentOpenError> {
        ...
        let id = if let Some(id) = id {
            ...
            if let Some(&(line, col)) = self.last_positions.get(&path) {
                ...
                doc.restore_position = Some(zmax_core::pos_at_coords(
                    text,
                    zmax_core::Position::new(line, col),
                    true,
                ));
            }
            ...
            zmax_event::dispatch(DocumentDidOpen {
                editor: self,
                doc: id,
            });
    ...
    pub fn close_document(&mut self, doc_id: DocumentId, force: bool) -> Result<(), CloseError> {
        ...
        let recorded = self.documents.get(&doc_id).and_then(|doc| {
            ...
            let text = doc.text().slice(..);
            let coords = zmax_core::coords_at_pos(text, sel.primary().cursor(text));
            Some((
                zmax_stdx::path::canonicalize(path),
        ...
        zmax_event::dispatch(DocumentDidClose { editor: self, doc });
    ...
    pub fn doc_diagnostics<'a>(
        ...
        document: &Document,
    ) -> impl Iterator<Item = zmax_core::Diagnostic> + 'a {
    ...
    pub fn enter_normal_mode(&mut self) {
        ...
        // vim has no persistent multi-cursor. A counted open (`3o`, `2O`) or a
        // block insert (`<C-v>jI`) fans out to one cursor per line so the typed
        // text lands on each, but on returning to Normal vim keeps a single
        // cursor — `2oXY<Esc>` or `<C-v>jI><Esc>` then any command must act on one
        // line, not all of them. zmax is helix-derived and would otherwise carry N
        // cursors out of Insert. The entry point already aimed the primary at
