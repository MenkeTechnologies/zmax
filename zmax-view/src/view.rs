...
// vim scrolling options. The `:set` option store lives in zmax-term, so the
// values are pushed down here (like `undolevels`); `usize::MAX` means "unset",
// which keeps zmax's own scrolling behaviour untouched.
...
/// soft-wrapped line. zmax has always scrolled by screen line (the view's
...
impl View {
    ...
    pub fn offset_coords_to_in_view_center<const CENTERING: bool>(
        ...
        if text_fmt.soft_wrap {
            ...
            // minimum needed (one column), which is what zmax always did.
...
mod tests {
    ...
    use zmax_core::{syntax, Rope};

    // 1 diagnostic + 1 marks + 1 spacer + 3 linenr (< 1000 lines) + 1 spacer + 1 diff
    ...
    /// vim `jumpoptions`: with `stack` (zmax's own behaviour) a jump made from
