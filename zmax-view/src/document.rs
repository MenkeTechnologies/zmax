...
use thiserror;
use zmax_core::auto_pairs::AutoPairs;
use zmax_core::chars::char_is_word;
use zmax_core::command_line::Token;
use zmax_core::diagnostic::DiagnosticProvider;
use zmax_core::doc_formatter::TextFormat;
use zmax_core::encoding::Encoding;
use zmax_core::snippets::{ActiveSnippet, SnippetRenderCtx};
use zmax_core::syntax::config::LanguageServerFeature;
use zmax_core::text_annotations::{InlineAnnotation, Overlay};
use zmax_event::TaskController;
use zmax_lsp::util::lsp_pos_to_pos;
use zmax_stdx::faccess::{copy_metadata, readonly};
use zmax_vcs::{DiffHandle, DiffProviderRegistry};
...
pub struct Document {
    ...
    pub(crate) text_props: zmax_core::text_props::TextProps,
    ...
    /// rendering via [`Document::text_format`]. See [`zmax_core::fold`].
    pub(crate) folds: zmax_core::fold::Folds,
    ...
    /// major modes are a superset of zmax's languages — `outline-mode` and
    ...
    /// on (see `zmax_term::keymap::major_mode`).
    ...
    // The mtime the file had as a result of zmax's most recent interaction with
    ...
    // post-write mtime here and does not mistake zmax's own write for an edit by
...
/// wants them but it shouldn't be Zmax who decides that so we use the most precise positioning.
///
...
/// leaves zmax's own detection (BOM, then `chardetng`) in charge.
...
/// reload (`:e!`, or a file changed behind zmax's back) instead of making the
...
/// value restores zmax's detection.
...
/// UTF-8; zmax falls back to its own detection). Pure — unit tested.
...
fn read_and_detect_encoding<R: std::io::Read + ?Sized>(
    ...
    let (encoding, has_bom) = encoding
        ...
        // order (including `ucs-bom`) before zmax's own detection.
        .or_else(|| {
...
use zmax_lsp::{lsp, Client, LanguageServerId, LanguageServerName};
use zmax_stdx::Url;
...
impl Document {
    pub fn from(
        ...
        Self {
            ...
            text_props: zmax_core::text_props::TextProps::new(),
            ...
            narrow: None,
            last_visual: None,
            folds: zmax_core::fold::Folds::default(),
            buftype: String::new(),
            color_swatches: None,
    ...
    pub fn format(
        ...
        if let Some((fmt_cmd, fmt_args)) = self
            ...
            .and_then(|formatter| {
                Some((
                    zmax_stdx::env::which(&formatter.command).ok()?,
                    &formatter.args,
                ))
            })
        {
            ...
            let formatting_future = async move {
                ...
                Ok(zmax_core::diff::compare_ropes(&text, &Rope::from(str)))
            };
        ...
        let fut = async move {
            let edits = request
                ...
                .unwrap_or_default();
            Ok(zmax_lsp::util::generate_transaction_from_edits(
                &text,
                edits,
                offset_encoding,
            ))
    ...
    > {
        ...
        let text = if self.major_mode() == Some("enriched") {
            ...
            Rope::from_str(&zmax_core::enriched::encode(&plain, &self.text_props))
        } else {
            self.text().clone()
        };

        let path = match path {
            Some(path) => zmax_stdx::path::canonicalize(path),
            None => {
        ...
        let future = async move {
            ...
            let is_hardlink = zmax_stdx::faccess::hardlink_count(&write_path).unwrap_or(2) > 1;
            ...
            // from the link state — which is what zmax always did.
            ...
            // running its external-modification guard and so won't flag zmax's
    ...
    pub fn reload(
        ...
        // of the encoding.
        let transaction = zmax_core::diff::compare_ropes(self.text(), &rope);
    ...
    pub fn set_path(&mut self, path: Option<&Path>) {
        let path = path.map(zmax_stdx::path::canonicalize);
    ...
    pub fn set_selection(&mut self, view_id: ViewId, selection: Selection) {
        ...
        if zmax_core::fold::foldclose_all() && !self.folds.is_empty() {
    ...
    fn apply_impl(
        ...
        zmax_event::dispatch(DocumentDidChange {
            doc: self,
            view: view_id,
            old_text: &old_doc,
            changes,
    ...
    /// (`zmax_term::keymap::major_mode`). The fallback is what makes an
    ...
    pub fn folds(&self) -> &zmax_core::fold::Folds {
        &self.folds
    }

    /// Mutable access to this document's folds (vim `zf`/`za`/...).
    pub fn folds_mut(&mut self) -> &mut zmax_core::fold::Folds {
        &mut self.folds
    }
    ...
    pub fn language_servers(&self) -> impl Iterator<Item = &zmax_lsp::Client> {
    ...
    ) -> impl Iterator<Item = &zmax_lsp::Client> {
        self.language_config().into_iter().flat_map(move |config| {
            config.language_servers.iter().filter_map(move |features| {
                let ls = &**self.language_servers.get(&features.name)?;
                if ls.is_initialized()
    ...
    pub fn uri(&self) -> Option<zmax_core::Uri> {
        Some(self.path()?.into())
    }
    ...
    pub fn relative_path(&self) -> Option<&Path> {
        self.relative_path
            .get_or_init(|| {
                self.path
                    .as_ref()
                    .map(|path| zmax_stdx::path::get_relative_path(path).to_path_buf())
            })
            .as_deref()
    }

    /// The workspace root for this document — the nearest ancestor that contains a `.git`, `.svn`,
    /// `.jj`, or `.zmax`. Falls back to the current working directory's workspace when the
    ...
    pub fn workspace_root(&self) -> &Path {
        self.workspace_root
            .get_or_init(|| match self.path.as_deref() {
                Some(p) => p
                    .parent()
                    .map(|dir| zmax_loader::find_workspace_in(dir).0)
    ...
    pub fn lsp_diagnostic_to_diagnostic(
        ...
        diagnostic: &zmax_lsp::lsp::Diagnostic,
        provider: DiagnosticProvider,
        offset_encoding: zmax_lsp::OffsetEncoding,
    ) -> Option<Diagnostic> {
        use zmax_core::diagnostic::{Range, Severity::*};
        ...
        use zmax_core::diagnostic::{DiagnosticTag, NumberOrString};
    ...
    pub fn undo_snapshot(&self) -> zmax_core::history::HistorySnapshot {
    ...
    pub fn restore_undo(&mut self, snapshot: zmax_core::history::HistorySnapshot) {
    ...
    /// `invisible` runs).
    pub fn text_props(&self) -> &zmax_core::text_props::TextProps {
        &self.text_props
    }
    ...
    pub fn update_text_props(&mut self, f: impl FnOnce(&mut zmax_core::text_props::TextProps)) {
    ...
    fn refresh_display_overlays(&mut self) {
        ...
        if self.prettify_symbols {
            ...
            {
                subs.extend(zmax_core::prettify::prettify(&text, symbols));
            }
        }
        if self.glyphless_display {
            subs.extend(zmax_core::prettify::glyphless_scan(&text));
        }
...
mod test {
    ...
    fn changeset_to_changes_ignore_line_endings() {
        use zmax_lsp::{lsp, Client, OffsetEncoding};
    ...
    async fn backup_keeps_previous_file_contents_on_save() {
        ...
        path.push(format!("zmax_backup_{}.txt", std::process::id()));
    ...
    fn is_changed_on_disk_tracks_external_writes() {
        ...
        let mut path = std::env::temp_dir();
        path.push(format!("zmax_changed_on_disk_{}.txt", std::process::id()));
    ...
    fn test_line_ending() {
        assert_eq!(
            ...
            .text()
            .to_string(),
            zmax_core::NATIVE_LINE_ENDING.as_str()
        );
    }
    ...
    macro_rules! encode {
        ($name:ident, $label:expr, $label_override:expr) => {
            ...
            fn $name() {
                ...
                zmax_lsp::block_on(to_writer(&mut buf, (encoding, false), &text)).unwrap();
    ...
    fn fileencodings_picks_the_first_encoding_that_decodes() {
        ...
        // Nothing listed => no opinion; zmax's own detection runs.
