//! Emacs `kbd`-syntax key descriptions → zemacs key tokens.
//!
...
//! (`x`, `RET`, `SPC`, `<f5>`, …). zemacs spells the same thing differently:
...
//! token to `zemacs_view::input::KeyEvent::from_str`, which is the authority.

/// Translate one Emacs key description (e.g. `C-M-x`, `RET`, `<f5>`) into the
/// zemacs token for the same key (`C-A-x`, `ret`, `F5`).
///
...
/// base key after it.
pub fn emacs_key_to_zemacs(desc: &str) -> Result<String, String> {
    if desc.is_empty() {
    ...
    loop {
        ...
        if bytes.len() >= 3 && bytes[1] == b'-' {
            let m = match bytes[0] {
                b'C' => Some("C"),
                b'M' | b'A' => Some("A"), // Emacs Meta / Alt → zemacs A-
                b'S' => Some("S"),
                _ => None,
            };
            match m {
    ...
    let base = base_key_to_zemacs(rest)?;
    if mods.is_empty() {
        Ok(base)
    } else {
...
fn base_key_to_zemacs(base: &str) -> Result<String, String> {
    ...
    // too, and zemacs spells it the same way).
...
/// into the sequence of zemacs key tokens it describes.
pub fn emacs_kbd_to_zemacs(spec: &str) -> Result<Vec<String>, String> {
    let parts: Vec<&str> = spec.split_whitespace().collect();
    if parts.is_empty() {
        return Err("empty key sequence".to_string());
    }
    parts.iter().map(|p| emacs_key_to_zemacs(p)).collect()
}
...
mod tests {
    ...
    fn plain_and_modified_keys() {
        assert_eq!(emacs_key_to_zemacs("x").unwrap(), "x");
        assert_eq!(emacs_key_to_zemacs("C-x").unwrap(), "C-x");
        assert_eq!(emacs_key_to_zemacs("M-x").unwrap(), "A-x");
        assert_eq!(emacs_key_to_zemacs("C-M-x").unwrap(), "C-A-x");
        // Shift stays S-, and a capital letter is left alone.
        assert_eq!(emacs_key_to_zemacs("S-tab").unwrap(), "S-tab");
        assert_eq!(emacs_key_to_zemacs("X").unwrap(), "X");
    }

    #[test]
    fn named_and_function_keys() {
        assert_eq!(emacs_key_to_zemacs("RET").unwrap(), "ret");
        assert_eq!(emacs_key_to_zemacs("SPC").unwrap(), "space");
        assert_eq!(emacs_key_to_zemacs("TAB").unwrap(), "tab");
        assert_eq!(emacs_key_to_zemacs("DEL").unwrap(), "backspace");
        assert_eq!(emacs_key_to_zemacs("<f5>").unwrap(), "F5");
        assert_eq!(emacs_key_to_zemacs("C-<f12>").unwrap(), "C-F12");
        assert_eq!(emacs_key_to_zemacs("<return>").unwrap(), "ret");
    }

    #[test]
    fn sequences_split_on_spaces() {
        assert_eq!(
            emacs_kbd_to_zemacs("C-c C-f").unwrap(),
            vec!["C-c".to_string(), "C-f".to_string()]
        );
        assert_eq!(
            emacs_kbd_to_zemacs("C-x 4 f").unwrap(),
    ...
    #[test]
    fn rejects_garbage() {
        assert!(emacs_kbd_to_zemacs("").is_err());
        assert!(emacs_key_to_zemacs("C-").is_err());
        assert!(emacs_key_to_zemacs("frobnicate").is_err());
        // A bare `-` is the minus key, not a dangling modifier.
        assert_eq!(emacs_key_to_zemacs("-").unwrap(), "minus");
    }
}
