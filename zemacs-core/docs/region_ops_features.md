# `zemacs-core::region_ops` — region / structural editing batch

Pure-Rust, editor-type-free algorithms in `zemacs-core/src/region_ops.rs`, each
...
single undoable transaction. Distinct from the `zemacs-term::text_ops` batch (no
...
| ⭐ `rotate_lines` | Cyclic wrap-around rotate of a line block by ±n | zemacs original — beyond Emacs, VS Code, Vim, Sublime, JetBrains, Zed, Helix |

Build/test: `cargo test -p zemacs-core` (green; region_ops adds 16 tests).
