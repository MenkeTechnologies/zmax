//! Workspace trust.
//!
//! Zmax can load workspace-local configuration (`.zmax/`) and launch language servers, both of
...
//! A grant snapshots a hash of every file under `.zmax/`. If those files change later the
...
//! multiple concurrent zmax instances writing trust for *different* workspaces (different
...
//! bypasses the `.zmax/` hash pin (changes to local config are never re-checked) and it trusts any
...
pub enum TrustQuery {
    ...
    Dap,
    /// Query whether `.zmax/` config can be loaded
    LocalConfig,
...
pub enum TrustStatus {
    ...
    Untrusted,
    /// Workspace was previously trusted, but the `.zmax/` tree has changed since the grant — the
...
pub enum ImplicitTrustLevel {
    ...
    None,
    /// Zmax-launched server processes (LSP and DAP) start implicitly. Workspace-local config and
...
pub fn build_trusted_globs(patterns: &[String]) -> GlobSet {
    ...
    for pattern in patterns {
        let expanded = zmax_stdx::path::expand(pattern);
...
struct CacheEntry {
    status: TrustStatus,
    /// Whether `.zmax/config.toml` or `.zmax/languages.toml` exists in the workspace at the time
...
impl WorkspaceTrust {
    /// Build the fully-trusted, no-prompt config that zmax always runs with.
    ///
    /// zmax deliberately does not implement the "trust this workspace" gate: there is no prompt
    ...
    /// disk. This catches external mutations to `.zmax/` that happened while zmax was running
    ...
    /// `.zmax/` changed) from *Untrusted* (never trusted)
    ...
    /// Mark `workspace` trusted. Snapshots the current `.zmax/` hash.
...
fn has_local_config(workspace: &Path) -> bool {
    workspace.join(".zmax").join("config.toml").exists()
        || workspace.join(".zmax").join("languages.toml").exists()
}

fn demote_for_query(status: TrustStatus, query: TrustQuery) -> TrustStatus {
    // Stale workspaces have their `.zmax/` config changed since trust was granted. LSP launches
...
// filename derives from the path, so concurrent zmax instances writing trust for *different*
...
/// SHA-256 of all files under `.zmax/`, used to detect changes to local config after trust was
/// granted. Returns `None` if `.zmax/` is absent or has no files, so a workspace with no local
/// config can still be trusted.
pub fn compute_workspace_hash(workspace: &Path) -> Option<String> {
    let zmax_dir = workspace.join(".zmax");
    if !zmax_dir.is_dir() {
        return None;
    }

    let mut files: Vec<PathBuf> = Vec::new();
    walk(&zmax_dir, &mut files);
    if files.is_empty() {
        return None;
    }
    files.sort();
    ...
    let mut hasher = Sha256::new();
    for file in &files {
        let rel = file.strip_prefix(&zmax_dir).unwrap_or(file);
...
fn walk(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if direct.is_dir() {
            ...
            // Symlinks to files (extremely common: dotfiles managers symlink `.zmax/config.toml`
...
mod test {
    ...
    #[test]
    fn hash_changes_when_zmax_dir_changes() {
        let dir = tempfile::tempdir().unwrap();
        let workspace = dir.path();

        let empty = compute_workspace_hash(workspace);
        assert_eq!(empty, None, ".zmax/ absent should hash to None");

        write_file(&workspace.join(".zmax").join("config.toml"), "a = 1");
        ...
        write_file(&workspace.join(".zmax").join("config.toml"), "a = 2");
        let h2 = compute_workspace_hash(workspace).expect("has files");
        assert_ne!(h1, h2);

        // Added file → different hash
        write_file(&workspace.join(".zmax").join("languages.toml"), "");
    ...
    fn set_config_invalidates_cache_so_stale_is_detected() {
        ...
        // after `:workspace-trust` was run. If the user (or another process) modified `.zmax/`
        // while zmax was running, subsequent queries kept returning Trusted even though
        ...
        // External mutation while zmax is running.
        write_file(&workspace.join(".zmax").join("config.toml"), "a = 2");
    ...
    fn hash_includes_symlinked_config_file() {
        // Regression: `.zmax/config.toml` is commonly a symlink to an external location (dotfiles
        ...
        fs::create_dir_all(workspace.join(".zmax")).unwrap();
        symlink(&external, workspace.join(".zmax").join("config.toml")).unwrap();
    ...
    fn hash_distinguishes_file_split() {
        ...
        let split = dir1.path();
        write_file(&split.join(".zmax").join("foo.toml"), "a");
        write_file(&split.join(".zmax").join("bar.toml"), "b");

        let dir2 = tempfile::tempdir().unwrap();
        let merged = dir2.path();
        write_file(&merged.join(".zmax").join("foo.toml"), "a\0bar.toml\0b");
