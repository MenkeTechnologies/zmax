...
/// without querying the terminal. Mirrors `zmax_term::true_color`, but lives
...
pub struct TerminaBackend {
    ...
    /// that custom colors set outside of Zmax with OSC11 are restored when Zmax exits.
...
impl Backend for TerminaBackend {
    fn claim(&mut self) -> io::Result<()> {
        ...
        write!(
            ...
            // things like mosh are buggy (upstream fix for terminals that don't clear).
            Csi::Edit(csi::Edit::EraseInDisplay(csi::EraseInDisplay::EraseDisplay)),
        )?;
